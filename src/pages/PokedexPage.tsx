      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg p-4 mb-6">
          <p className="text-center mb-2">
            コレクション: {caughtPokemon.length} / 1008 (
        ...
        <div className="flex justify-center mb-6">
          <button
            onClick={() => setShowCaughtOnly(false)}
            className={`btn px-6 py-2 text-lg font-bold mx-2 ${
              !showCaughtOnly ? "btn-primary" : "btn-secondary"
            }`}
          >
            ポケモン図鑑
          </button>
          <button
            onClick={() => setShowCaughtOnly(true)}
            className={`btn px-6 py-2 text-lg font-bold mx-2 ${
              showCaughtOnly ? "btn-primary" : "btn-secondary"
            }`}
          >
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4 relative">
          {(showCaughtOnly
            ? caughtPokemon.map((pokemon) => pokemon.id)
            : Array.from({ length: 1008 }, (_, i) => i + 1)
          ).map((id) => {
            const pokemon = caughtPokemon.find((p) => p.id === id);
            return (
              <motion.div
                key={pokemon ? pokemon.id : id}
