...
const PokemonGacha: React.FC = () => {
  ...
  const [isAnimating, setIsAnimating] = useState(false);
  const [revealProgress, setRevealProgress] = useState(0);
  const [hasPulled, setHasPulled] = useState(false); // ガチャ済みフラグを追加
  const { addPokemon } = usePokedex();

  const handleGacha = useCallback(async () => {
    if (hasPulled) return; // すでにガチャを引いていたら何もしない

    setHasPulled(true); // ガチャ済みに設定
    setIsRevealed(false);
    setIsAnimating(true);
    setRevealProgress(0);
    setPokemon(null);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-sky-300 to-sky-500 overflow-hidden p-4">
      ...
      {/* <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mt-6">
        <button
          onClick={handleGacha}
          disabled={isAnimating || hasPulled} // ガチャ済みならボタン無効化
          className="btn btn-primary px-6 py-2 text-lg font-bold"
        >
          {hasPulled ? "ガチャ終わり" : "ガチャを引く"}
        </button>
        <Link
          to="/pokedex"
          className="btn btn-secondary px-6 py-2 text-lg font-bold"
